    ...
    <div className="space-y-6 p-4 md:p-6 text-left">
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        <button
          ...
          className="flex items-center gap-2 text-xs font-bold text-secondary hover:text-primary transition group"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-bronze font-mono uppercase tracking-wider font-bold">Delegate ID Active</span>
        </div>
      </div>

      {/* HEADER CARD WITH 2PX BRONZE BORDER & GLOW SHADOW */}
      <div className="bg-card border-2 border-bronze rounded-[18px] p-6 space-y-5 shadow-[0_0_40px_rgba(154,107,50,0.2)] relative overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <img
                src={user.avatar}
                alt="Avatar"
                className="w-16 h-16 rounded-full bg-background border-2 border-bronze p-1 shadow-inner"
              />
              <span className="absolute bottom-0 right-0 w-4 h-4 rounded-full bg-background border border-bronze flex items-center justify-center text-[10px]">
                🪳
              </span>
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-black text-primary">{user.anonymousName}</h1>
                {user.role === 'ADMIN' && (
              ...
              <p className="text-xs text-bronze font-mono font-bold mt-1.5 flex items-center gap-1.5 flex-wrap">
          ...
          {!isEditing ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-background border border-border text-primary hover:border-bronze font-bold py-2 px-4 rounded-[10px] text-xs transition flex items-center gap-2 shadow-sm"
              >
                <Edit2 size={14} />
              ...
              <button
                onClick={logout}
                title="Logout"
                className="bg-background border border-red-500/30 text-red-400 hover:bg-red-500/10 py-2 px-3 rounded-[10px] text-xs transition flex items-center gap-1 font-semibold"
              >
                <LogOut size={14} />
                <span>Logout</span>
              </button>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-background border border-border text-muted hover:text-primary py-2 px-3 rounded-[10px] text-xs transition flex items-center gap-1"
              >
                <X size={14} />
                <span>Cancel</span>
              </button>
              <button
                ...
                disabled={saving}
                className="bg-primary text-background font-black py-2 px-4 rounded-[10px] text-xs transition flex items-center gap-1 hover:opacity-90 border border-bronze"
              >
                <Save size={14} />
        ...
        {isEditing ? (
            ...
            <div>
              ...
              >
                {delegateTags.map((tag) => (
                  <option key={tag} value={tag} className="bg-card text-primary">
                    {tag}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              >
                {politicalHouses.map((h) => (
                  <option key={h} value={h} className="bg-card text-primary">
                    {h}
                  </option>
                ))}
              </select>
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2 font-mono text-xs">
              <div className="bg-background border border-border rounded-[10px] p-3 text-center">
                <p className="text-sm font-black text-bronze">{userPosts.length}</p>
                <p className="text-[10px] text-muted">Motions Raised</p>
              </div>
              <div className="bg-background border border-border rounded-[10px] p-3 text-center">
                <p className="text-sm font-black text-primary">{user.college || 'Lok Sabha (Lower House)'}</p>
                <p className="text-[10px] text-muted">Parliament Bench</p>
              </div>
              <div className="bg-background border border-emerald-500/40 rounded-[10px] p-3 text-center col-span-2 sm:col-span-1 shadow-[0_0_15px_rgba(16,185,129,0.15)]">
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-extrabold text-sm uppercase tracking-wider text-primary">Your Raised Motions & Debates</h2>
          </div>
          <span className="text-xs text-muted font-mono">{userPosts.length} Motions</span>
        </div>
        ...
        {loadingPosts ? (
          ...
          <div className="bg-card border border-border rounded-[14px] p-8 text-center space-y-3">
            {/* <span className="text-3xl block"></span> */}
            <h3 className="font-bold text-sm text-primary">No Motions Raised Yet</h3>

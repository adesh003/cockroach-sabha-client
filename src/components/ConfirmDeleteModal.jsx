...
export default function ConfirmDeleteModal({ isOpen, onClose, onConfirm, deleting }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-[#171717] border-2 border-red-500/40 rounded-[20px] w-full max-w-sm p-6 space-y-5 shadow-[0_0_50px_rgba(239,68,68,0.25)] text-left relative animate-fadeIn">
        ...
        <div className="flex items-center justify-between border-b border-[#292929] pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-sm uppercase text-white tracking-wider">Withdraw Motion</h3>
              <p className="text-[10px] text-red-400 font-mono">Sabha Floor Action</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="text-[#71717A] hover:text-white p-1 rounded-full hover:bg-[#292929] transition"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-[#292929]">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-[9px] text-xs font-bold border border-[#292929] text-[#A1A1AA] hover:text-white hover:bg-[#292929] transition"
          >
            Keep Motion
          </button>
          <button
